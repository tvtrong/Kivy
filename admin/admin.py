...
class AdminWindow(BoxLayout):
    ...
    def add_product_fields(self):
        ...
        crud_purchase = TextInput(
            hint_text="Product Purchase", multiline=False)
        crud_submit = Button(
            text="Add",
    ...
    def add_product(self, code, name, weight, stock, sold, order, purchase):
        if code == '' or name == '' or weight == '' or stock == '' or order == '':
            ...
            self.products.insert_one({"product_code": code, "product_name": name, "product_weight": weight,
                                      "in_stock": stock, "sold": sold, "order": order, "last_purchase": purchase})
    ...
    def update_user_fields(self):
        ...
        crud_des = Spinner(text="Operator", values=[
                           "Operator", "Administrator"])
        crud_submit = Button(text="Update", background_color=(189/255.0, 126/255.0, 24/255.0, 1), size_hint_x=None, width=100, on_release=lambda x: self.update_user(
            crud_first.text, crud_last.text, crud_user.text, crud_pwd.text, crud_des.text))
        ...
        if code == '':
            ...
            if target_code == None:
                ...
                self.products.update_one(
                    {"product_code": code},
                    {"$set": {"product_code": code, "product_name": name, "product_weight": weight,
                              "in_stock": stock, "sold": sold, "order": order, "last_purchase": purchase}},
                )
                prodz = self.get_products()
                stocktable = DataTable(table=prodz)
    ...
    def remove_user_fields(self):
        ...
        crud_submit = Button(text="Remove", size_hint_x=None, width=100, background_color=(189/255.0, 43/255.0, 24/255.0, .9),
    ...
    def remove_product_fields(self):
        ...
        crud_submit = Button(text="Remove", background_color=(189/255.0, 43/255.0, 24/255.0, .9), size_hint_x=None, width=100,
    ...
    def remove_user(self, user):
        if user == '':
            ...
            if target_user == None:
                self.notify.add_widget(
                    Label(text='[color=#FF0000][b]Invalid User Name[/b][/color]', markup=True))
                self.notify.open()
                Clock.schedule_once(self.killswitch, 1)
            else:
                content = self.ids.scrn_contents
    ...
    def remove_product(self, code):
        if code == '':
            ...
            if target_code == None:
                self.notify.add_widget(
                    Label(text='[color=#FF0000][b]Invalid Product Code[/b][/color]', markup=True))
                self.notify.open()
                Clock.schedule_once(self.killswitch, 1)
            else:
                content = self.ids.scrn_product_contents
    ...
    def get_users(self):
        ...
        for user in users.find():
            ...
            first_names.append(user.get("first_name"))
            last_names.append(user.get("last_name"))
            user_names.append(user.get("user_name"))
            l = user.get("password")
            if len(l) > 10:
                l = l[:10] + "..."
            passwords.append(l)
            designations.append(user.get("designation"))
        # print(designations)
